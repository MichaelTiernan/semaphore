...
usage()
{
	...
	fprintf (stderr, "Note: Only PIDs >1 will be killed.\n\n");
...
main (int argc, char **argv, char **env)
{
	sem_t *sem;
	char *semname, *sfname;
	int opt, mode=0, timo, ret, dowrite;
	int semfile, wr, nr;
	int mysem_size;
	int semstat;
	int dokill;
	...
	while ((opt = getopt(argc, argv, "s:chrtk:l:Tp")) != -1) {
		DEBUGPRT (8, "opt=%d %c\n", opt, opt);
		switch (opt) {
			case 's' :
				if (mode == 0) {
	...
	switch (mode) {
		case 's' :
			...
			if (tdif < 0) {
				DEBUGPRT (2, "Invalid time %d semaphore '%s'\n", tdif, semname);
				mysem.ms_counter = 0;
			} 
			if (mysem.ms_counter < 0) {
				DEBUGPRT (2, "Invalid counter %d semaphore '%s'\n", mysem.ms_counter, semname);
				mysem.ms_counter = 0;
			}
...
/* makes no sense...
						...
						DEBUGPRT (1, "killing my parent (%d) of semaphore '%s'\n", me_ppid, semname);
						// kill parent
						if (me_ppid  > 1) {
							kill (me_ppid,  abs(dokill));
						}
*/
					} // 0 = no kill
				}
			...
			DEBUGPRT (4, "set counter = %d, ret=%d for '%s'\n", mysem.ms_counter, ret, semname);
			break;
		case 'c' :
			ret = 0;
			if (mysem.ms_counter > 0) {
			...
			DEBUGPRT (4, "clear counter = %d for '%s'\n", mysem.ms_counter, semname);
			dowrite = 1;
			break;
		case 'r':
			if (mysem.ms_counter == 0) {
